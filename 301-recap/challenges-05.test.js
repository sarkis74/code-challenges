...
const splitFoods = (recipe) => {
    ...
    for(var i in recipe.ingredients) {
      ...
      if(recipe.ingredients[i].split(' ')[3] != undefined) {
        result[i]+= ' ' + recipe.ingredients[i].split(' ')[3];
      }
      if(recipe.ingredients[i].split(' ')[4] != undefined)
        result[i]+= ' ' + recipe.ingredients[i].split(' ')[4];
    }
    return result;
  }
